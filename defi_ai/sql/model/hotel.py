...
class Hotel(SQLBase):
    ...
    def get_count_statement(cls):
        hotel_city_count = (
            select(cls.city, func.count(cls.id)).group_by(cls.city).subquery()
        )
        hotel_brand_count = (
            select(cls.brand, func.count(cls.id)).group_by(cls.brand).subquery()
        )
        hotel_group_count = (
            select(cls.group, func.count(cls.id)).group_by(cls.group).subquery()
        )
        hotel_city_group_count = (
            select(cls.city, cls.group, func.count(cls.id))
            .group_by(cls.city, cls.group)
            .subquery()
        )
        hotel_city_brand_count = (
            select(cls.city, cls.brand, func.count(cls.id))
